...
class DriverController extends Controller
{
    ...
    function status_work(Request $request)
    {
        $id_driver = (int)$request->id_driver;
    ...
    function barang_diambil(Request $request)
    {
        ...
        if ((int)$count > 0) {
            $data = array(
                'status' => 6,
                'tgl_barang_diambil' => $tgl,
            );
            if (!empty($path_img)) {
                $randomletter = substr(str_shuffle("kebutKEBUT"), 0, 5);
                $nama_file = 'ambilBarang' . $id_transaksi;
                $nama_file = base64_encode($nama_file . '' . $randomletter);
                $fileSize = $path_img->getSize();
                $extension = $path_img->getClientOriginalExtension();
                $imageName = $nama_file . '.' . $extension;
                $tujuan_upload = 'uploads/transaksi/ambil_barang';
                ...
                $data += array("img_barang_diambil" => env('PUBLIC_URL') . '/uploads/transaksi/ambil_barang/' . $imageName);
            }
    ...
    function barang_diserahkan(Request $request)
    {
        $tgl = date('Y-m-d H:i:s');
        $id_driver = (int)$request->id_driver;
        $id_transaksi = (int)$request->id_transaksi > 0 ? (int)$request->id_transaksi : 0;
        ...
        if ((int)$count > 0) {
            ...
            if (!empty($path_img)) {
                $randomletter = substr(str_shuffle("kebutKEBUT"), 0, 5);
                $nama_file = 'barangDiserahkan' . $id_transaksi;
                $nama_file = base64_encode($nama_file . '' . $randomletter);
                $fileSize = $path_img->getSize();
                $extension = $path_img->getClientOriginalExtension();
                $imageName = $nama_file . '.' . $extension;
                $tujuan_upload = 'uploads/transaksi/barang_diserahkan';
                ...
                $path_img->move($tujuan_upload, $imageName);
                $data += array("img_barang_diserahkan" => env('PUBLIC_URL') . '/uploads/transaksi/barang_diserahkan/' . $imageName);
            }
