...
class TransaksiController extends Controller
{
	...
	public function trans_available(Request $request)
    {
        ...
        if (in_array($sort_column, $column_int)) $sort_column = "ABS($sort_column)";
        $sort_column = $sort_column . " " . $sort_order;
        $where = array('transaksi.status' => $status, 'id_driver'=>null);
    ...
    function store(Request $request)
    {
		...
		$ttl_biaya_asuransi = $request->has('ttl_biaya_asuransi') ? str_replace(',', '',$request->ttl_biaya_asuransi) : 0; 
		$ttl_biaya_bm = $request->has('ttl_biaya_bm') ? str_replace(',', '',$request->ttl_biaya_bm) : 0; 
		$sub_ttl = $request->has('sub_ttl') ? str_replace(',', '',$request->sub_ttl) : 0; 
		...
		$dt_trans = array(
			...
			'ttl_biaya_asuransi'	=> $ttl_biaya_asuransi,
			...
			'ttl_biaya'				=> $ttl_biaya + (int)$kode_unik_transfer,
			'status'				=> $status,
			'created_at'			=> $tgl,
		);
		$id = 0;
		...
		if((int)$id > 0){
			...
			if($payment == 2) {
				...
				DB::table('transaksi')->where('id_transaksi', $id)->update(array('url_payment' => $url_payment,'signature'=>$signature));
